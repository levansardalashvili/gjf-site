...
// გვერდების საკუთარი კონტენტი (სიახლეების ტექსტი და ა.შ.) თითოეულ კომპონენტში
// ცალკეა გატარებული (მაგ. NewsDetailContent, ProjectCard) — item.title_en/body_en
// ველების არსებობისას.
export const DICT = {
  ka: {
...
export function LanguageProvider({ children, initialLang = "ka" }) {
  ...
  useEffect(() => {
    ...
    if ((saved === "en" || saved === "ka") && saved !== initialLang) setLangState(saved);
  }, [initialLang]);

  function setLang(next) {
    setLangState(next);
    if (typeof window !== "undefined") {
      localStorage.setItem("gjf_lang", next);
      // cookie-შიც ვინახავთ (არა მხოლოდ localStorage-ში), რომ სერვერულმა
      // კომპონენტებმაც (generateMetadata, <html lang>) იცოდნენ არჩეული ენა.
      document.cookie = `gjf_lang=${next}; path=/; max-age=31536000; SameSite=Lax`;
    }
  }
