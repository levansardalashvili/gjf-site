...
// contact_messages — RLS-ის გამო service_role კლიენტით (მხოლოდ admin-ისთვის)
...
// live_broadcast — ერთი, "სინგლტონ" ჩანაწერი ლაივის მდგომარეობისთვის
export async function getLiveBroadcast() {
  const { data } = await supabase.from("live_broadcast").select("*").order("id").limit(1).single();
  return data;
}
