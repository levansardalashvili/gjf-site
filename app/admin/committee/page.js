        ...
        {members.map((m) => (
          <div key={m.id} className="flex items-center gap-4 px-5 py-4 border-b border-line last:border-0">
            ...
            <div className="min-w-0 flex-1">
              ...
              {m.role && <div className="text-sm opacity-60 truncate">{m.role}</div>}
            </div>
            <div className="flex items-center gap-4 shrink-0">
              <Link href={`/admin/committee/${m.id}`} className="text-sm text-gold font-semibold">რედაქტირება</Link>
              <DeleteButton endpoint={`/api/committee/${m.id}`} confirmText={`წავშალო "${m.name}"?`} />
            </div>
          </div>
        ))}
