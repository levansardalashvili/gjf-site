...
export default function CalendarRegulationForm({ initial, id }) {
  ...
  const [form, setForm] = useState({
    ...
    force_download: initial?.force_download || false,
  ...
  async function handleSubmit(e) {
    ...
    setSaving(true);
    setError("");
    const endpoint = isEdit ? `/api/calendar-regulations/${id}` : "/api/calendar-regulations";
    ...
    if (res.ok) {
      ...
      router.push("/admin/calendar-regulations");
      router.refresh();
    } else {
      const data = await res.json();
    ...
    <form onSubmit={handleSubmit} className="max-w-xl flex flex-col gap-4">
      ...
      <div>
        <label className="block text-xs uppercase tracking-wide opacity-55 mb-1.5">
          სრული ტექსტი (არასავალდებულო — თუ შეავსებ, ეს ჩანაწერი გვერდზე დიდ, გამოკვეთილ
          "შეტყობინების" ბლოკად გამოჩნდება, ჩვეულებრივი პატარა ბარათის ნაცვლად)
        </label>
        <RichTextEditor value={form.body} onChange={(html) => update("body", html)} />
      </div>

      <div>
        <label className="block text-xs uppercase tracking-wide opacity-55 mb-1.5">დოკუმენტი (PDF ან Excel)</label>
        {form.file_url ? (
      ...
      <label className="flex items-center gap-2 text-sm">
        <input type="checkbox" checked={form.force_download} onChange={(e) => update("force_download", e.target.checked)} />
        დაჭერისთანავე ავტომატურად ჩამოტვირთოს (და არა ბრაუზერში გახსნას)
      </label>
