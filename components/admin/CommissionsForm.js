...
export default function CommissionsForm({ initial, id }) {
  ...
  async function handleSubmit(e) {
    ...
    setSaving(true);
    setError("");
    const endpoint = isEdit ? `/api/commissions/${id}` : "/api/commissions";
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-xl flex flex-col gap-4">
      ...
      <div>
        <label className="block text-xs uppercase tracking-wide opacity-55 mb-1.5">კომისიის დასახელება</label>
        <input className={FIELD_CLASS} value={form.role} onChange={(e) => update("role", e.target.value)} required />
      </div>
