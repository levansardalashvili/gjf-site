...
export default function ResultDetailContent({ result }) {
  const { lang } = useLanguage();
  const backHref = result.category === "georgia" ? "/results/georgia" : "/results/international";
  const eventName = (lang === "en" && result.event_name_en) ? result.event_name_en : result.event_name;

  return (
    <>
      <div className="text-sm opacity-50 pt-8 pb-2">
        <a href="/results" className="hover:text-gold"><Trans k="results" /></a> /{" "}
        <a href={backHref} className="hover:text-gold"><Trans k={CAT_KEY[result.category]} /></a> / {eventName}
      </div>
      ...
      <div className="my-6 rounded-2xl overflow-hidden bg-gradient-to-br from-crimson to-crimson-dark p-7 text-white">
        ...
        <h1 className="font-serif font-bold text-2xl md:text-4xl leading-tight mb-3">{eventName}</h1>
